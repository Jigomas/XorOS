...
ELF-загрузчика. Стек в `0x2000`, растёт вниз в пределах 64 KiB.
...
│   └── csr.h         — адреса CSR-регистров и коды исключений (заготовка для trap)
├── src/
│   ├── boot.S        — точка входа _start: инициализация SP, вызов kernel_main
│   └── kernel_main.c — ядро: print() + "Hello from XorOS!\n" + sys_exit()
├── tests/
...
> CSR-инструкции (`csrr`/`csrw`) в симуляторе пока не реализованы.
