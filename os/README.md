...
```plaintext
  Автомат состояний процесса:
     ...
     │      │  READY   │
     │      └────┬─────┘
     │           │ context_switch (становится текущим)
     │           ▼
     │      ┌──────────┐
     │      │ RUNNING  │
     │      └────┬─────┘
     │           │ sched_yield()         │ sched_exit()
     └───────────┘ (уступает квант)      │
                                         ▼
                                    ┌──────────┐
                                    │  ZOMBIE  │  слот занят до реализации wait/reap
                                    └──────────┘
  Таймерное прерывание → sched_yield() → RUNNING переходит в READY
```
