...
**ECALL + MMIO.** Низкоуровневые системные вызовы (`sys_putchar`, `sys_exit`) реализованы
через `ecall` — симулятор перехватывает их через syscall table (`a7` — номер, `a0`–`a6` —
аргументы). UART-драйвер (`uart.c`) работает через MMIO: запись байта по адресу `0xF000`
перехватывается симулятором и выводится в stdout.
...
│   ├── trap.S          — trap_entry: сохранение caller-saved, вызов trap_handler, mret
...
- [ ] уровни привилегий в CPU — хранить текущий режим M/S/U; Illegal Instruction при CSR-доступе из U-режима
...
- [ ] PTE_U — флаги пользовательских страниц
