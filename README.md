...
│  0xF000 UART TX · 0xF004–08 mtime · 0xF00C–10 mtimecmp     │     (перехватывается симулятором)
├─────────────────────────────────────────────────────────────┤
...
│  └─ procs[] содержит стеки процессов (512 байт + канарейка) │  ← sp при переключении
├─────────────────────────────────────────────────────────────┤
│  DATA (инициализированные глобалы)                          │  ← gp ± 2 KiB
├─────────────────────────────────────────────────────────────┤
│  RODATA  строковые литералы                                 │
├─────────────────────────────────────────────────────────────┤
│  TEXT                        0x0000                         │  ← pc · ra
...
Низкие адреса
  ...
  sp    — верхушка стека текущего процесса (в BSS, растёт вниз)
...
| `spinlock.h`     | Header-only spinlock через `amoswap.w.aq` / `amoswap.w.rl` (A-расширение)   |
